...
], DiceRoll.prototype, "result", void 0);
__decorate([
    property({ type: String })
...
DiceRoll = __decorate([
    ...
    renderCustomProperties() {
        if (!this.data.custom || Object.keys(this.data.custom).length === 0)
            return null;
        return html `
      ${Object.keys(this.data.custom).map((key) => {
            return html `<li class="label success">${key}: ${this.data.custom[key]}</li>`;
        })}
    `;
    }
    render() {
