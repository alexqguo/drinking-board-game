...
LitElement.render = render$1;let PokemonSelector = class PokemonSelector extends LitElement {
    ...
    handleDone() {
        const results = this.playerNames.map((n) => this.selectedPokemon[n]);
        const e = new CustomEvent('pokemon-selected', {
            detail: {
                pokemon: results,
            },
        });
    ...
    render() {
      ...
      <div style="font-size: 1rem">
        ${this.playerNames.map((name) => this.renderPlayerSelection(name))}
...
const starterStrengths = {
    ...
    isBattleOver() {
        const incompletePlayers = this.data.filter((player) => {
    ...
    battleOver() {
        let winners = [];
        let maxRoll = null;
        Object.keys(this.results).forEach((playerName) => {
            const playerMax = Math.max(...this.results[playerName]);
        ...
        Object.keys(this.results).forEach((playerName) => {
            if (Math.max(...this.results[playerName]) === maxRoll) {
                winners.push(this.data.find((p) => p.playerName === playerName));
            }
        });
    ...
    handleRoll(e) {
        ...
        const player = e.currentTarget.dataset.playerName;
    ...
    renderWinCondition() {
      ...
      `;
        ...
        const winnerNames = this.winners.map((winner) => winner.playerName);
        return html `
    ...
    render() {
        return html `
      <div style="font-size: 1rem">
        ${this.data.map((player) => this.renderPlayer(player))}
    ...
    firstUpdated() {
        this.data.forEach((player) => {
            const weakPokemon = starterStrengths[player.starterName];
            const hasStrength = this.data.filter((p) => p.starterName === weakPokemon).length > 0;
...
], TrainerBattle.prototype, "data", void 0);
__decorate([
    property({ type: String })
], TrainerBattle.prototype, "winners", void 0);
TrainerBattle = __decorate([
    customElement('trainer-battle')
], TrainerBattle);
var TrainerBattle$1 = TrainerBattle;console.info(`[pokemon-gen1] including web components ${PokemonSelector$1.name}, ${TrainerBattle$1.name}`);
const w = window;
if (w.drinking) {
    const { Game, GameEvents, events } = w.drinking;
    GameEvents.on(events.GAME_START, (next) => {
    ...
    function triggerTrainerBattle(players) {
        return new Promise(resolve => {
            const frag = document.createDocumentFragment();
            const battle = document.createElement('trainer-battle');
            const componentArgs = [...players].map((p) => {
                return {
                    playerName: p.name,
            ...
            battle.addEventListener('battle-ended', (e) => {
